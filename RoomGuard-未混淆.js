...
const RG_CLOUD_KEY = "RoomGuardData";       
const RG_CLOUD_MAX_BYTES = 150000;          
const RG_OBF_KEY = "RoomGuard#R131#BondageClub#2026";
...
function RGObfuscate(str) {
	const key = RG_OBF_KEY;
	const utf8 = [];
...
function RGCompactAction(a) {
	return { t: a.t, p: a.target, k: a.kind, c: a.change, g: a.group, i: a.item || "" };
}

 
function RGCompactLogEntry(e) {
	return {
		t: e.t, n: e.num, m: e.name || "", k: e.nick || "", r: e.reason || "",
		x: (Array.isArray(e.actions) ? e.actions : []).map(RGCompactAction),
		b: e.banned ? 1 : 0,
	};
}

 
function RGNormalizeAction(a) {
	if (!a || typeof a !== "object") return null;
	const target = a.target != null ? a.target : a.p;
	if (target == null) return null;
	return {
		t: a.t,
		target,
		kind: a.kind != null ? a.kind : (a.k || "clothes"),
		change: a.change != null ? a.change : (a.c || "other"),
		group: a.group != null ? a.group : (a.g || ""),
		item: a.item != null ? a.item : ((a.i != null && a.i !== "") ? a.i : null),
	};
}

 
function RGNormalizeLogEntry(e) {
	if (!e || typeof e !== "object") return null;
	const num = e.num != null ? e.num : e.n;
	if (num == null) return null;
	const rawActions = Array.isArray(e.actions) ? e.actions : (Array.isArray(e.x) ? e.x : []);
	return {
		t: e.t,
		num,
		name: e.name != null ? e.name : (e.m || ""),
		nick: e.nick != null ? e.nick : (e.k || ""),
		reason: e.reason != null ? e.reason : (e.r || ""),
		actions: rawActions.map(RGNormalizeAction).filter(Boolean),
		banned: e.banned != null ? !!e.banned : !!e.b,
	};
}

 
function RGSerializeState(state, includeLog) {
	const out = { v: 1, settings: state.settings };
	if (includeLog) {
		out.kickCount = state.kickCount || {};
		out.banCount = state.banCount || {};
		out.log = (state.log || []).map(RGCompactLogEntry);
	}
	return out;
}

 
function RGLZCompress(json) {
	try {
...
function RGDecodeRaw(raw) {
	if (typeof raw !== "string" || !raw) return null;
	let text = raw;
	if (raw.indexOf("p:") === 0) {
		try { text = RGLZDecompress(RGDeobfuscate(raw.slice(2))); } catch (e) {   }
	} else if (raw.indexOf("z:") === 0) {
		text = RGLZDecompress(raw.slice(2));
	}
	try { return JSON.parse(text); } catch (e) { return null; }
}

 
function RGLocalEncode(state) {
	const json = JSON.stringify(RGSerializeState(state, true));
	const comp = RGLZCompress(json);
	return (comp != null) ? ("z:" + comp) : json;
}

 
function RGCloudEncode(state, includeLog) {
	const json = JSON.stringify(RGSerializeState(state, includeLog));
	const comp = RGLZCompress(json);
	if (comp != null) return "p:" + RGObfuscate(comp);
	return json;
}

 
function RGCloudDecode(raw) {
	return RGDecodeRaw(raw);
}
...
function RGDefaultState() {
	return {
		...
		settings: {
			...
			msgWardrobe: "提示：{0} 进房后一句话没说就打开了衣柜，即将被移出房间。",
			cloudStoreLog: false, 
		},
		kickCount: {},           
		banCount: {},            
...
function RGNormalizeState(raw) {
	...
	if (raw && typeof raw === "object") {
		...
		st.condWardrobe = s.condWardrobe !== false;
		st.wardrobeIndependent = s.wardrobeIndependent !== false;
		...
		st.wardrobeMaxSpoken = Number.isFinite(Number(s.wardrobeMaxSpoken)) && Number(s.wardrobeMaxSpoken) >= 1 ? Math.floor(Number(s.wardrobeMaxSpoken)) : 1;
		...
		st.msgWardrobe = typeof s.msgWardrobe === "string" ? s.msgWardrobe : st.msgWardrobe;
		st.cloudStoreLog = s.cloudStoreLog === true;
		...
		d.log = Array.isArray(raw.log) ? raw.log.slice(0, RG_LOG_CAP).map(RGNormalizeLogEntry).filter(Boolean) : [];
	}
	return d;
}
...
const RGStore = {
	...
	load() {
		...
		this.state = RGNormalizeState(RGDecodeRaw(raw));
		return this.state;
	},
	...
	save() {
		...
		this.saveTimer = setTimeout(() => {
			...
			try {
				const key = this.baseKey();
				if (!key) return;
				const existing = RGDecodeRaw(RGStorage.get(key));
				const merged = RGMergeState(existing, this.state);
				this.state = merged;
				RGStorage.set(key, RGLocalEncode(merged));
				this.cloudSave();
	...
	cloudSave() {
		try {
			if (typeof ServerAccountUpdate === "undefined" || !ServerAccountUpdate || typeof ServerAccountUpdate.QueueData !== "function") return false;
			if (typeof Player === "undefined" || !Player || typeof Player.CharacterID !== "string" || Player.CharacterID === "") return false;
			const includeLog = !!this.state.settings.cloudStoreLog;
			const payload = RGCloudEncode(this.state, includeLog);
			if (RGUTF8Bytes(payload) > RG_CLOUD_MAX_BYTES) return false;
			const obj = {};
			obj[RG_CLOUD_KEY] = payload;
	...
	flush() {
		...
		try {
			const key = this.baseKey();
			if (!key) return;
			RGStorage.set(key, RGLocalEncode(this.state));
			this.cloudSave();
...
function RGTriggeredKeys(s) {
	...
	if (st.condWardrobe && s.c4) keys.push("wardrobe");
	return keys;
}
...
function RGCondsTrigger(s) {
	...
	if (st.condWardrobe && st.wardrobeIndependent && s.c4) return true;
	
	const enabled = [];
	if (st.condAge) enabled.push(!!s.c1);
	if (st.condClothes) enabled.push(!!s.c2);
	if (st.condRestraint) enabled.push(!!s.c3);
	if (st.condWardrobe && !st.wardrobeIndependent) enabled.push(!!s.c4);
...
function RGNotifySend(s, key) {
	const st = RGStore.state.settings;
	const map = { age: "msgAge", clothes: "msgClothes", restraint: "msgRestraint", wardrobe: "msgWardrobe" };
...
function RGOnJoin(charData, sourceNum) {
	...
	const s = {
		...
		c1: false, c2: false, c3: false, c4: false, kicked: false, kickPending: false, actions: [],
	};
	RGComputeConds(s);
...
function RGOnMessage(data) {
	...
	if (data.Type === "Status" && data.Content === "Wardrobe") {
		if (!s.kicked && !s.kickPending && (Number(s.spokenCount) || 0) < RGStore.state.settings.wardrobeMaxSpoken) {
			s.c4 = true;
			RGMaybeTrigger(s);
		}
		return;
	...
	try {
		mod.hookFunction("LoginResponse", 10, (args, next) => {
			...
			safe(() => {
				...
				if (typeof Player === "undefined" || !Player || !Number.isInteger(Player.MemberNumber) || Player.MemberNumber <= 0) return;
				RGStore.ensureAccount(Player.MemberNumber);
				if (typeof C[RG_CLOUD_KEY] === "string") {
					try {
						const cloudObj = RGCloudDecode(C[RG_CLOUD_KEY]);
						if (cloudObj && typeof cloudObj === "object") {
							
							if (cloudObj.settings && typeof cloudObj.settings === "object") {
								RGStore.state.settings = RGNormalizeState({ settings: cloudObj.settings }).settings;
							}
							if (cloudObj.log != null || cloudObj.kickCount != null || cloudObj.banCount != null) {
								const full = RGNormalizeState(cloudObj);
								RGStore.state.kickCount = full.kickCount;
								RGStore.state.banCount = full.banCount;
								RGStore.state.log = full.log;
							}
							const key = RGStore.baseKey();
							if (key) RGStorage.set(key, RGLocalEncode(RGStore.state));
							RGUIRefresh();
							RGUI.renderLog();
						}
					} catch (e) { RGErr("读取云端数据失败", e); }
				}
			})();
			return res;
		});
	} catch (e) { RGErr("hook LoginResponse 失败", e); }
}
...
const RGText = {
	zh: {
		...
		condWardrobe: "未说话就打开衣柜",
		condWardrobeTitle: "进房后一句话没说就打开衣柜（外观编辑界面）即触发",
		wardrobeIndependent: "衣柜条件独立判定（不参与多条件逻辑）",
		wardrobeIndependentTitle: "勾选：进房直接打开衣柜就立即踢出，不受「满足其一/其二/同时满足」影响；不勾选则衣柜条件也参与多条件判定",
		clothesMaxSpoken: "改衣服：说话少于多少句触发",
		restraintMaxSpoken: "上束缚：说话少于多少句触发",
		wardrobeMaxSpoken: "开衣柜：说话少于多少句触发",
		...
		msgWardrobeLabel: "开衣柜条件提示消息",
		msgPlaceholder: "消息里可用 {0} 表示对方昵称/名字",
		msgDelaySeconds: "消息延迟（秒）",
		kickDelaySeconds: "踢出延迟（秒）",
		cloudStoreLog: "日志也存云端（默认只存配置）",
		cloudStoreLogTitle: "默认云端只保存设置；勾选后才把踢出日志与计数也上传到账号数据",
	...
	en: {
		...
		condWardrobe: "Opens wardrobe without speaking",
		condWardrobeTitle: "Trigger when they open the wardrobe (appearance editor) without saying a word",
		wardrobeIndependent: "Judge wardrobe independently (not part of combine logic)",
		wardrobeIndependentTitle: "Checked: kick immediately when they open the wardrobe, ignoring Any/Two/All; unchecked: wardrobe joins the combine logic",
		clothesMaxSpoken: "Clothes: trigger if spoken fewer than N",
		restraintMaxSpoken: "Restraint: trigger if spoken fewer than N",
		wardrobeMaxSpoken: "Wardrobe: trigger if spoken fewer than N",
		...
		msgWardrobeLabel: "Wardrobe condition notice",
		msgPlaceholder: "Use {0} for the player's name",
		msgDelaySeconds: "Message delay (sec)",
		kickDelaySeconds: "Kick delay (sec)",
		cloudStoreLog: "Store log in cloud too (config always stored)",
		cloudStoreLogTitle: "By default only settings go to the cloud; check this to also upload the kick log and counters",
		...
		reasonWardrobe: "Opened wardrobe without speaking",
		kindClothes: "Clothes",
...
function RGReasonLabel(reasonKeys) {
	const map = { age: "reasonAge", clothes: "reasonClothes", restraint: "reasonRestraint", wardrobe: "reasonWardrobe" };
	const parts = [];
...
function RGUIBuildSettings(root) {
	...
	root.appendChild(RGCheckboxRow("rg-set-condWardrobe", RGT("condWardrobe"), "change", (v) => {
		RGStore.state.settings.condWardrobe = v; RGCondChanged();
	}, RGT("condWardrobeTitle")));
	root.appendChild(RGCheckboxRow("rg-set-wardrobeIndependent", RGT("wardrobeIndependent"), "change", (v) => {
		RGStore.state.settings.wardrobeIndependent = v; RGCondChanged();
	}, RGT("wardrobeIndependentTitle")));
	...
	root.appendChild(RGNumberRow("rg-set-wardrobeMaxSpoken", RGT("wardrobeMaxSpoken"), 1, 100, (v) => {
		RGStore.state.settings.wardrobeMaxSpoken = v; RGStore.save();
	}));
	...
	RGUI.msgSection.appendChild(RGTextRow("rg-set-msgWardrobe", RGT("msgWardrobeLabel"), (v) => { RGStore.state.settings.msgWardrobe = v; RGStore.save(); }));

	
	root.appendChild(RGCheckboxRow("rg-set-cloudStoreLog", RGT("cloudStoreLog"), "change", (v) => {
		RGStore.state.settings.cloudStoreLog = v; RGStore.save();
	}, RGT("cloudStoreLogTitle")));
...
function RGMultiCondCount() {
	const s = RGStore.state.settings;
	let n = 0;
	if (s.condAge) n++;
	if (s.condClothes) n++;
	if (s.condRestraint) n++;
	if (s.condWardrobe && !s.wardrobeIndependent) n++;
	return n;
}
...
function RGUIRenderCombine() {
	...
	if (RGMultiCondCount() >= 3) options.push({ value: "two", label: RGT("combineTwo") });
...
function RGCondChanged() {
	const s = RGStore.state.settings;
	if (s.combine === "two" && RGMultiCondCount() < 3) {
		s.combine = "any";
	}
	RGStore.save();
	RGUI.renderSettings();
...
RGUI.renderSettings = function () {
	...
	const cw = get("rg-set-condWardrobe"); if (cw) cw.checked = !!s.condWardrobe;
	const wi = get("rg-set-wardrobeIndependent"); if (wi) wi.checked = !!s.wardrobeIndependent;
	...
	const wm = get("rg-set-wardrobeMaxSpoken"); if (wm) wm.value = String(s.wardrobeMaxSpoken);
	...
	const mw = get("rg-set-msgWardrobe"); if (mw) mw.value = s.msgWardrobe || "";
	const cl = get("rg-set-cloudStoreLog"); if (cl) cl.checked = !!s.cloudStoreLog;
...
if (typeof module !== "undefined" && module.exports) {
	module.exports = {
		...
		RGCloudEncode, RGCloudDecode, RGDecodeRaw, RGLocalEncode, RGSerializeState, RGLZCompress, RGLZDecompress,
		RGCompactLogEntry, RGCompactAction, RGNormalizeLogEntry, RGNormalizeAction, RGUTF8Bytes, RGObfuscate, RGDeobfuscate, RG_CLOUD_KEY,
